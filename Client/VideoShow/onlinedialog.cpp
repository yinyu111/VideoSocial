...
void OnlineDialog::slot_loginRs(char *buf, int nlen)
{
    ...
    switch( rs ->m_lResult )
    {
        ...
        case login_sucess :
            ...
            //界面
            ui->lb_name->setText( QString("%1") .arg(m_userName));
            m_login->hide();
            m_id = rs->m_UserID ;
...
void OnlineDialog::slot_fileBlockRq(char *buf, int nlen)
{
    ...
    for(int i=10; i>info->fileId; i--)
    {
        QString pbNum1 = QString("pb_play%1").arg(i);
        QMyMovieLabel * pb_play1 = ui->sw_page->findChild<QMyMovieLabel *>(pbNum1);
        pb_play1->clear();
    }
...
void OnlineDialog::slot_PlayClicked()
{
    ...
    qDebug()<< pb_play->rtmpUrl();
    ui->le_rtmpUrl->setText(pb_play->rtmpUrl());
...
void OnlineDialog::UploadFile(QString filePath, Hobby hy , QString gifName  )
{
    ...
    qDebug() <<"rq.m_szFileName="<<rq.m_szFileName ;
    ...
    qDebug() <<"rq.m_nFileId="<<rq.m_nFileId ;
    qDebug() <<"rq.m_nFileSize="<<rq.m_nFileSize ;
    qDebug() <<"m_nType="<<rq.m_nType ;
    qDebug() <<"rq.m_szFileName="<<rq.m_szFileName ;
    qDebug() <<"rq.m_szFileType="<<rq.m_szFileType ;
    qDebug() <<"rq.m_szGifName="<<rq.m_szGifName ;
    qDebug() <<"rq.m_UserId="<<rq.m_UserId ;
...
void OnlineDialog::on_pb_rtmpPlay_clicked()
{
    QString cmd;
    cmd.clear();
    cmd = ui->le_rtmpUrl->text();
    Q_EMIT SIG_PlayUrl(  cmd );
}
